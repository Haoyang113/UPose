...
using UnityEngine;
// using UnityEngine.ProBuilder.MeshOperations; // 这个引用似乎未使用，可以注释掉以减少依赖
...
public class UPose : MonoBehaviour, MotionTrackingPose
{
    ...
    private void Start()
    {
        ...
        t.IsBackground = true; // 设置为后台线程，主程序退出时它也会退出
        t.Start();
    }
    private void Update()
    {
    ...
    private void Run()
    {
        ...
        while (true)
        {
            ...
            {
                // 使用 + 号进行字符串拼接，以提高兼容性
                Debug.LogError("An error occurred in Run loop: " + ex.Message);
                Thread.Sleep(100);
            }
        }
